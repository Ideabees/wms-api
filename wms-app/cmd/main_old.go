...
func verifyOTPHandler(c *gin.Context) {
	...
	// save token in db
	_, dbErr = repository.SetToken(token, req.MobileNumber)
	if dbErr != nil {
		fmt.Println("Error while saving token in db", dbErr)
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to feed token"})
		return
	}
...
func main_old() {
	...
	v1 := r.Group("/api/wms/v1")
	{
		...
		//v1.POST("/permissions", authMiddleware(), permissionsHandler)
		v1.POST("/sign-up", controllers.Register)
